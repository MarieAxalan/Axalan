...
mean(B.bonddata$BOND.AMOUNT)^2
mean(W.bonddata$BOND.AMOUNT)^2
mean(H.bonddata$BOND.AMOUNT)^2
mean(A.bonddata$BOND.AMOUNT)^2
mean(df$BOND.AMOUNT)^2


xbar<- mean(B.bonddata$BOND.AMOUNT)
s<-sd(B.bonddata$BOND.AMOUNT)
n<-nrow(B.bonddata)

t= (xbar-mean(df$BOND.AMOUNT))/(s/sqrt(n))
